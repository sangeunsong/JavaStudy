 ...
 *           예) 파일명이 틀리다,IP가 틀리다,URL주소가 틀리다,0으로 입력,배열 인덱스 초과...
 *    ==>컴파일 에러/실행 시 에러(UnCheckException) =>예외처리 필수(선택)
 *       --------  --------
 *       CheckedException =>컴파일러가 확인(반드시 예외처리를 해야한다)
 *    ==> 1.에러를 사전에 방지하는 프로그램
 ...
 *    ==>예외처리 방법
 *       -예외처리 =>소스 변경X,복구해서 실행해 주는 것은 아니다,에러 부분을 잘라낸다
 *                 --------1
 *                 --------2
 *                 =======================>정상수행
 *                 --------3 =>error
 ...
 *                    ===>try = if,catch = else
 *       -직접처리(예외복구)
 *           try{정상적으로 실행되는 문장(오류가 발생할 수도 있다,실수가 있을 수 있다)}
 *           catch{실수한 부분을 처리} =>여러개 사용이 가능(순서가 존재 =>계층구조)
 *               catch절을 한개만 사용한다면(Exception,Trowable)
 *           finally{필수적으로 닫기(자원 반환}
 *       -간접처리(예외회피)
 *           throws =>예상되는 예외를 선언한다
 *                    -------------------예측이 가능하기 때문에 나중에 사용 시 대처하기 쉽다
 *                    사용자 정의는 거의 없다,라이브러리 메소드에서 예측
 *           =>선언하는 방법
 *           =>try~catch
 *         
 *       기타적
